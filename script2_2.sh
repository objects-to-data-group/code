...
cat nytimes_humanities_tabbed.csv | awk -F '\t' '{print $1}' | grep - | sed -e 's/.//8g' | sed -e 's/.*-//' | sort | uniq -c | awk '{print $1 "\t" $2}' > artikelen_per_maand.txt
...
cat nytimes_humanities_tabbed.csv | awk -F '\t' '{if ($2 ~ "Front Page" || $8 ~ "Front Page") print $1}' | sed -e 's/.//8g' | sed -e 's/.*-//' | sort | uniq -c | awk '{print $1}' > voorpagina_artikelen_per_maand.txt
